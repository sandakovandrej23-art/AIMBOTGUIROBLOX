...
local function createToggle(name, settingKey, order)
    ...
    toggleBtn.BackgroundColor3 = aimbotSettings[settingKey] and Color3.fromRGB(50, 168, 82) or Color3.fromRGB(120, 120, 120)
    ...
    toggleBtn.MouseButton1Click:Connect(function()
        aimbotSettings[settingKey] = not aimbotSettings[settingKey]
        toggleBtn.BackgroundColor3 = aimbotSettings[settingKey] and Color3.fromRGB(50, 168, 82) or Color3.fromRGB(120, 120, 120)
    end)
...
createToggle("Check Walls", "CheckWalls", 1)
createToggle("Use Command", "UseCommand", 2)

local rangeContainer = Instance.new("Frame")
rangeContainer.Name = "RangeContainer"
rangeContainer.Size = UDim2.new(0.8, 0, 0, 40)
rangeContainer.Position = UDim2.new(0.1, 0, 0.8, 0)
...
madeByText.Position = UDim2.new(0, 0, 0.95, 0)
...
local function checkWalls(targetPosition)
    if not aimbotSettings.CheckWalls then return true end
    ...
    local raycastResult = Workspace:Raycast(cameraPosition, direction * 1000, raycastParams)
...
local function checkCommand()
    if not aimbotSettings.UseCommand then return true end
    
    -- Проверка нажатия клавиши (например, LeftAlt)
...
local function getClosestPlayerInRange()
    ...
    local closestDistance = aimbotSettings.AimRange
        ...
        if otherPlayer ~= player and otherPlayer.Character and otherPlayer.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if humanoid and humanoid.Health > 0 and rootPart then
                ...
                if onScreen then
                    ...
                    if distance <= aimbotSettings.AimRange and distance < closestDistance then
                        -- Проверяем стены и команду
                        if checkWalls(rootPart.Position) and checkCommand() then
...
rangeBox.FocusLost:Connect(function(enterPressed)
    ...
    if newRange then
        aimbotSettings.AimRange = math.clamp(newRange, 10, 200)
        rangeBox.Text = tostring(aimbotSettings.AimRange)
        
        crosshairFrame.Size = UDim2.new(0, aimbotSettings.AimRange * 2, 0, aimbotSettings.AimRange * 2)
        crosshairFrame.Position = UDim2.new(0.5, -aimbotSettings.AimRange, 0.5, -aimbotSettings.AimRange)
    else
        rangeBox.Text = tostring(aimbotSettings.AimRange)
    end
end)
...
toggleButton.MouseEnter:Connect(function()
    local tween = TweenService:Create(toggleButton, TweenInfo.new(0.2), {BackgroundTransparency = 0.2})
    tween:Play()
end)

toggleButton.MouseLeave:Connect(function()
    local tween = TweenService:Create(toggleButton, TweenInfo.new(0.2), {BackgroundTransparency = 0})
    tween:Play()
end)

minimizeButton.MouseEnter:Connect(function()
    local tween = TweenService:Create(minimizeButton, TweenInfo.new(0.2), {BackgroundTransparency = 0.2})
    tween:Play()
end)

minimizeButton.MouseLeave:Connect(function()
    local tween = TweenService:Create(minimizeButton, TweenInfo.new(0.2), {BackgroundTransparency = 0})
    tween:Play()
end)
